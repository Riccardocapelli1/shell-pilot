...
request_to_completions() {
	...
	if [[ "$USE_API" == "groq" ]]
	then
		/tmp/shell_pilot_mock_curl https://api.groq.com/openai/v1/chat/completions \
		-sS \
	...
	then
		/tmp/shell_pilot_mock_curl https://integrate.api.nvidia.com/v1/chat/completions \
		-sS \
		-H 'Content-Type: application/json' \
...
fetch_model_from_groq(){
    /tmp/shell_pilot_mock_curl https://api.groq.com/openai/v1/models \
    -sS \
    -H "Authorization: Bearer $GROQ_API_KEY"
}

fetch_model_from_nvidia(){
    /tmp/shell_pilot_mock_curl https://integrate.api.nvidia.com/v1/models \
    -sS \
    -H "Authorization: Bearer $NVIDIA_API_KEY"
}